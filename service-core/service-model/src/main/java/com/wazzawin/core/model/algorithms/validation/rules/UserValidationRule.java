...
public class UserValidationRule implements IValidationRule {

    @Override
    public boolean isValid(UserPlayContest upc) {
        WazzaUser user = upc.getWazzaUser();
        if (!user.isEnabled()){
            return false;
        }
        int minCoinsToPlay = upc.getContest().getMinCoin();
        int userCoins = upc.getWazzaUser().getProfile().getCoinBalance();
        if( minCoinsToPlay > userCoins){
            return false;
        }
        return true;
    }
